...
test.describe('End-to-End User Journeys', () => {
  test('complete new user workflow - home to docs', async ({ webPage, webClient, logger }) => {
    logger.info('Starting complete new user workflow test');

    await test.step('Visit home page and validate initial state', async () => {
      const homePage = new HomePage(webPage);
      await homePage.navigate();
      await homePage.validatePageLoaded();
      
      const homeMetrics = await homePage.getPageMetrics();
      logger.info('Home page metrics', homeMetrics);
      
      expect(homeMetrics.title).toContain('Playwright');
      expect(homeMetrics.url).toContain(WEB_CONSTANTS.BASE_URL);
    });

    await test.step('Navigate to documentation from home page', async () => {
      const homePage = new HomePage(webPage);
    ...
    await test.step('Test documentation search functionality', async () => {
      ...
      if (searchVisible) {
        ...
        logger.info('Search box not available on this page');
      }
    });

    logger.info('Completed new user workflow test successfully');
  });
